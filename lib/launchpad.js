const LP = {
  ...
  // Color Functions
  setButtonColor: function (buttonId, colorId = 0, type = 'solid') {
  ...
  setGroupColor: function (buttonIds, colorId, type) {
    for (let id of buttonIds) LP.setButtonColor(id, colorId, type);
  },
  setRowColor: function (rowId, colorId, type) {
    if (rowId < 0 || rowId > 8) throw 'Invalid row id. Must be between 0 - 8.';

    if (rowId === 8) {
      LP.setTopRowColor(colorId, type);
      return;
    }

    const buttonIds = [1,2,3,4,5,6,7,8].map(n => n+(((8-rowId))*10));
    LP.setGroupColor(buttonIds, colorId, type);
  },
  setColumnColor: function (colId, colorId, type) {
    if (colId < 0 || colId > 8) throw 'Invalid column id. Must be between 0 - 8.';
    const buttonIds = [1,2,3,4,5,6,7,8].map(n => (n*10)+(colId+1));
    LP.setGroupColor(buttonIds, colorId, type);
  },
  setGridColor: function (colorId, type) {
    LP.setGroupColor(LP.buttons.grid, colorId, type);
  },
  setRightColumnColor: function (colorId, type) {
    LP.setGroupColor(LP.buttons.right, colorId, type);
  },
  setTopRowColor: function (colorId, type) {
    LP.setGroupColor(LP.buttons.top, colorId, type);
  },
  setArrowColor: function (colorId, type) {
    LP.setGroupColor(LP.buttons.arrow, colorId, type);
  },
  setModeColor: function (colorId, type) {
    LP.setGroupColor(LP.buttons.mode, colorId, type);
  },
  setQuadrantColor: function (quadrantId, colorId, type) {
    if (quadrantId < 0 || quadrantId > 3) throw 'Invalid quadrant id. Must be between 0 - 3.';
    LP.setGroupColor(LP.buttons.quadrant[quadrantId],colorId,type);
  },
  setAllColor: function (colorId, type) {
    LP.setGroupColor(LP.buttons.all, colorId, type);
  },
  clear: function () {
    LP.setGroupColor(LP.buttons.all);
  },
  clearGrid: function () {
    LP.setGroupColor(LP.buttons.grid);
  },
};
