...
function onPress(msg) {
  if (LP.buttons.mode.includes(msg.data[1])) {
    ...
    LP.setButton(msg.data[1], currentColor);
    updateLP();
  }
}
...
function onMessage(msg) {
  if (msg.data[0] === 144) {
    ...
    LP.setButton(msg.data[1], (msg.data[2] ? currentColor : 0));
  }
}
...
function updateLP() {
  ...
  for (let r in grid) {
    for (let c in grid[r]) {
      if (grid[r][c]) LP.setButton((((7-r) + 1.0)*10)+(c*1 + 1.0), currentColor);
    }
  }
}
...
const modeTypeFuncs = {
  'gate': (row,col,data,grid) => {
    if (col === 8) {
      LP.setRow(row, (data[2] ? currentColor : 0));
      LP.setButton(data[1], (data[2] ? currentColor : 0));
  ...
  'toggle': (row,col,data,grid) => {
    if (data[2]) {
      if (col === 8) {
        LP.setButton(data[1], currentColor);
        grid[row] = grid[row].map(el => el ? 0 : 1);

        const arr = [1,2,3,4,5,6,7,8].map(el => el+((8-row)*10));
        for (let i in arr) {
          LP.setButton(arr[i], grid[row][i] ? currentColor : 0);
        }
      } else {
        LP.setButton(data[1], (grid[row][col] ? 0 : currentColor));
  ...
  'pan': (row,col,data,grid) => {
    if (data[2] && col < 8) {
      LP.setRow(row); // Clear row
      grid[row].fill(0);
      if (col < 3) {
        let i = data[1];
        while (i%10 < 5) {
          LP.setButton(i,currentColor);
          grid[row][(i%10) - 1] = 1;
          i++;
        }
      } else if (col > 4) {
        ...
        LP.setButton(((8-row)*10)+4,currentColor);
        LP.setButton(((8-row)*10)+5,currentColor);
        grid[row][3] = 1;
        grid[row][4] = 1;
      }
    }
  ...
  'fader': (row,col,data,grid) => {
    if (data[2]) {
      if (col === 8) {
        ...
        for (let i = (7-row); i >= 0; i--) LP.setButton(((i+1)*10)+col+1, currentColor);
      }
    } else if (col === 8) {
      LP.setButton(data[1]);
    }
